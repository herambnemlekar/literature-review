...
## Symbolic Representation & Planning
...
1. [B. Ames, A. Thackston, and G.D. Konidaris. Learning Symbolic Representations for Planning with Parameterized Skills. Proceedings of the 2018 IEEE/RSJ International Conference on Intelligent Robots and Systems, October 2018.](http://irl.cs.brown.edu/pubs/param_skill_symbols.pdf)
	- Abstract: Construct a representation suitable for planning with parametrized motor skills. Specify conditions which are sufficient to separate the selection of motor skills from the parametrization of those skills. Our method results in a simple discrete abstract representation for planning followed by a parameter selection process that operates on a fixed plan.
	- Problem: Robot must select both the motor skill to execute at a particular time and additionally select appropriate parameters for it.
	- Assumptions: 1) To generate a discrete representation there should be finite number of symbols needed to represent the preconditions and effects. This is true for subgoal skills whose effect is independent of the start state. Skills can be partitioned into subgoal skills. 2) Skill intersections are Lipschitz continuous. Therefore the difference between the true maximum probability of skill intersection and the probability from sampled points would be bounded. So we can approximate the image of a motor skill with its effect.
	- Method: Search the set of motor skill parameters which maximize a given plan’s probability of success. In practice, find θ which maximizes the probability of Inter(oj,θj,oi,Z) for all o∈O.
	- Limitations: 1) Does not discuss how a plan is found in the first place, before the optimal parameters for it can be found. If they do joint optimization over parameters and plans, then it is not mentioned. 2) Does not tell how a parameterized action can be partitioned into subgoal skills. 3) Does not consider the case where the effect of the parameterized action is dependent on the start state.
...
## Other methods for abstract representation and planning
